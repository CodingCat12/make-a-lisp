...
pub fn parse_expr(input: &str) -> IResult<&str, EvalTo<String>> {
    alt((
        map(parse_int_expr, |o| -> EvalTo<String> {
            Box::new(Format(o.eval()))
        }),
        map(parse_float_expr, |o| -> EvalTo<String> {
            Box::new(Format(o.eval()))
        }),
    ))(input)
}
...
#[derive(Debug)]
struct Format<T: Expr<T>>(T);

impl<T: Expr<T>> Expr<String> for Format<T> {
    fn eval(&self) -> String {
        format!("{:#?}", self.0.eval())
    }
}
